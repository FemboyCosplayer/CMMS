...
const MYSQL_URL = "mysql://root:vqpyEePZSKwQatgkyNdNQKmbZKgtuxDI@mysql.railway.internal:3306/railway";
...
async function runMigration() {
  ...
  try {
    ...
    if (tables.length > 0) {
      console.log(`[v0] Database already has ${tables.length} tables. Skipping migration.`);
      console.log('[v0] Tables found:', tables.map(t => t.TABLE_NAME).join(', '));
    } else {
      console.log('[v0] No tables found. Running migration...');
      ...
      for (let i = 0; i < statements.length; i++) {
        try {
          console.log(`[v0] [${i + 1}/${statements.length}] Executing statement...`);
